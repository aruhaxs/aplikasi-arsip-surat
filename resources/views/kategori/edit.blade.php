...
@section('content')
    ...
    <form action="{{ route('kategori.update', $kategori->id) }}" method="POST">
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="text" id="id" name="id" disabled value="{{ $kategori->id }}" style="width: 100px; padding: 8px; background-color: #e9ecef;">
        </div>
        <div style="margin-bottom: 15px;">
            <label for="nama_kategori" style="display: block;">Nama Kategori</label>
            <input type="text" id="nama_kategori" name="nama_kategori" required value="{{ $kategori->nama_kategori }}" style="width: 300px; padding: 8px;">
        </div>
        <div style="margin-bottom: 15px;">
            <label for="keterangan" style="display: block;">Judul</label>
            <textarea id="keterangan" name="keterangan" rows="4" required style="width: 600px; padding: 8px;">{{ $kategori->keterangan }}</textarea>
        </div>
